...
LaunchAgent (`com.claude-rtl.watcher`) that rebuilds the copy after every Claude update.

Layout:

- `mac/install.sh` — macOS installer (idempotent). `mac/swap-when-quit.sh` — helper.
- `linux/` — Linux installer. If it only contains `PROMPT.md`, the Linux port has not been
  done yet; see the Linux section.
...
Claude Desktop for Linux is an official beta (Ubuntu 22.04+/Debian 12+, apt repo).

- If `linux/install.sh` exists: read `linux/README.md` (or the header of the script) and use
  it the same way as the macOS installer: `--check`, then `--install`, then `--status`, then
  tell the user what to verify visually.
- If `linux/` contains only `PROMPT.md`: the port has not been done yet. Follow the prompt
  in `linux/PROMPT.md` — it explains how the macOS installer works and what the Linux
  installer must do — build it, test it on this machine, commit it to this repository under
  `linux/`, update this section of `CLAUDE.md`, and push.
